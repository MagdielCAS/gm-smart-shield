...
from gm_shield.features.knowledge.tasks import (
    run_sheet_extraction,
    run_reference_extraction,
)
...
def sheet_source_id(db_session):
    source = KnowledgeSource(
        file_path="/tmp/DungeonMasterGuide.pdf", status="completed", current_step="Done"
    )
...
def run_sheet_extraction_task(db_session, sheet_source_id, monkeypatch):
    ...
    monkeypatch.setattr(
        "gm_shield.features.knowledge.tasks.SheetAgent", lambda: mock_agent
    )

    # Patch SessionLocal to use our test session
    monkeypatch.setattr(
        "gm_shield.features.knowledge.tasks.SessionLocal", lambda: db_session
    )
...
def verify_sheet_template_exists(db_session, sheet_source_id):
    template = (
        db_session.query(CharacterSheetTemplate)
        .filter_by(source_id=sheet_source_id)
        .first()
    )
...
def ref_source_id(db_session):
    source = KnowledgeSource(
        file_path="/tmp/Spells.txt", status="completed", current_step="Done"
    )
    db_session.add(source)
    db_session.commit()
    db_session.refresh(source)
...
def verify_reference_extraction(db_session, ref_source_id):
    ref = (
        db_session.query(QuickReference)
        .filter_by(source_id=ref_source_id, name="Fireball")
        .first()
    )
